...
if __name__ == "__main__":
    ...
    action_noise = None
    if config["action_noise"] == "const":
        action_noise = config["scale"]
    elif config["action_noise"] == "linear":
        action_noise = (config["scale"], 0.0003, 250_000)
    elif config["action_noise"] == "ornstein":
        action_noise = ("or", config["scale"])
    ...
    if isinstance(rl_options["action_noise"], tuple) and len(rl_options["action_noise"]) == 3:
        ...
        action_noise = None
    elif isinstance(rl_options["action_noise"], tuple) and len(rl_options["action_noise"]) == 2:
        n_actions = env.action_space.shape[-1]
        action_noise = OrnsteinUhlenbeckActionNoise(mean=np.zeros(n_actions), sigma=float(rl_options["action_noise"][1]) * np.ones(n_actions))
