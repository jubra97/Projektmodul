...
if __name__ == "__main__":
    ...
    if isinstance(config["method"], list):
        ...
        if isinstance(config["method"][1], int):
            length = config["method"][1]
        else:
            obs_config = config["method"][1]
    else:
    ...
    if length:
        observation_options["history_length"] = length
    if obs_config:
        obs_config_dict = {}
        for extra in config["method"][1]:
            obs_config_dict[extra] = True
        observation_options["obs_config"] = obs_config_dict
    observation_options["use_u"] = False
