...
class ControllerParams(gym.Env, abc.ABC):
    ...
    def _create_obs_errors_with_vel(self):
        ...
        u_change = (system_inputs[-20:-10] - system_inputs[-10:]) * 1 / self.output_freq
        input_vel = np.mean(u_change)
        ...
        obs = [error_smooth[-1],  error_vel * 10]
    ...
    def _create_reward_discrete(self):
        ...
        if self.oscillation_pen_fun:
            pen_action = self.oscillation_pen_fun(abs((1/pen_error) * u_change)) * self.oscillation_pen_gain
        else:
            pen_action = abs(u_change) * self.oscillation_pen_gain
    ...
    def _create_reward_discrete2(self):
        ...
        if self.oscillation_pen_fun:
            pen_action = self.oscillation_pen_fun(abs(action_change)) * 20
        else:
