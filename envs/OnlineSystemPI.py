...
class OnlineSystemPI:
    def __init__(self):
        ...
        self.ads_buffer_sps = self.plc.get_symbol("Object4 (RL_Learn_PIControl).Output.ads_buffer", array_size=20)
        ...
        self.input_pi = self.plc.get_symbol("Object4 (RL_Learn_PIControl).Input.IN_P_I")
...
if __name__ == "__main__":
    ...
    with env.ads_buffer_mutex:
        plt.plot(env.last_t, env.last_w, label="w")
        plt.plot(env.last_t, env.last_u, label="u")
        plt.plot(env.last_t, env.last_y, label="y")
        mean_y = [np.mean(env.last_y[i - 200:i]) if i > 200 else np.mean(env.last_y[0:i]) for i in
