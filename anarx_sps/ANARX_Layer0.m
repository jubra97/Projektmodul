function [x5, state] = ANARX_Layer0(input, params, varargin)
%ANARX_LAYER0 Function implementing an imported ONNX network.
%
...
%
% [X5] = ANARX_Layer0(INPUT, PARAMS)
%			- Evaluates the imported ONNX network ANARX_LAYER0 with input(s)
%			INPUT and the imported network parameters in PARAMS. Returns
%			network output(s) in X5.
%
% [X5, STATE] = ANARX_Layer0(INPUT, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = ANARX_Layer0(INPUT, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
[Vars, NumDims] = packageVariables(params, {'input'}, {input}, [1]);
% Call the top-level graph function:
[x5, x5NumDims, state] = torch_jit_exportGraph1000(input, NumDims.input, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x5] = postprocessOutput(x5, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x5, x5NumDims1002, state] = torch_jit_exportGraph1000(input, inputNumDims1001, Vars, NumDims, Training, state)
...
% MatMul:
[Vars.x4, NumDims.x4] = onnxMatMul(Vars.input, Vars.x6, NumDims.input, NumDims.x6);

% Add:
Vars.x5 = Vars.linear_layers_0_bias + Vars.x4;
NumDims.x5 = max(NumDims.linear_layers_0_bias, NumDims.x4);

% Set graph output arguments from Vars and NumDims:
x5 = Vars.x5;
x5NumDims1002 = NumDims.x5;
% Set output state from Vars:
state = updateStruct(state, Vars);
end

function [inputDataPerms, outputDataPerms, Training] = parseInputs(input, numDataOutputs, params, varargin)
% Function to validate inputs to ANARX_Layer0:
p = inputParser;
...
input = permuteInputVar(input, inputDataPerms{1}, 1);
% Check input size(s):
checkInputSize(size(input), {2}, "input");
end

function [x5] = postprocessOutput(x5, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
...
x5 = permuteOutputVar(x5, outputDataPerms{1}, 1);
end
