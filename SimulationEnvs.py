...
class FullAdaptivePT2(gym.Env):
    def __init__(self, oscillating=False, model_sample_frequency=10_000, controller_sample_frequency=100,
        ...
        self.observation_space = gym.spaces.Box(low=np.array([-11, -1000]), high=np.array([11, 1000]), shape=(2,), dtype=np.float32)
    ...
    def reset(self):
        ...
        :return: (np.array)
        """
        step_height = random.uniform(0.1, 10)
        u_before_step = [0] * int(0.5 * self.model_sample_frequency)
    ...
    def step(self, action):
        ...
        if self.simulation_time_steps >= len(self.t):
            ...
            reward = -np.sum(np.square(np.array(self.out) - self.u))
            reward = reward / len(self.t)
            # self.render()
        else:
            done = False
            reward = -np.square(self.out[self.simulation_time_steps-1] - self.u[self.simulation_time_steps-1])
            # reward = 0
